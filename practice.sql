...
--- practice task ---
--- https://like-frog-b41.notion.site/SQL-Join-Practice-Task-27ac979408f5477da80de4ab299f9225 ---
...
SELECT * FROM employees                   --- Inner Join to Retrieve Employee and Department Information
JOIN departments USING(department_id);

SELECT department_name, round(avg(salary)) FROM employees
JOIN departments USING(department_id)                         --- Group By Department with Average Salary
GROUP BY department_name
;

SELECT department_name, count(department_id) FROM employees
JOIN departments USING(department_id)                         --- Count Employees in Each Department
GROUP BY department_name
;

SELECT department_name, round(avg(salary)) as avg_salary FROM employees
JOIN departments USING(department_id)                         --- Find the Department name with the Highest Average Salary
GROUP BY department_name
ORDER BY avg_salary DESC
LIMIT 1
;

SELECT extract(year FROM hire_date) as hire_year, count(*) FROM employees  --- Count Employees Hired Each Year
GROUP BY hire_year;
